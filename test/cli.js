...
export async function cliTest(_fixtures) {
  suite("CLI", () => {
    ...
    test("Componentize", async () => {
      // Clear outDir before this test, as it involves transpilation
      try {
        await rm(outDir, { recursive: true });
      } catch {}
    ...
    test("Componentize (weval)", async () => {
      ...
      await cp(`${outDir}/componentize.js`, "/tmp/componentize.js");
      await cp(`${outDir}/foo.js`, "/tmp/foo.js");
      await cp(
        `${outDir}/componentize.core.wasm`,
        "/tmp/componentize.core.wasm",
      );
      const m = await import(`${pathToFileURL(outDir)}/componentize.js`);
      strictEqual(m.hello(), "world");
      // TODO: create for class bar can only be called once??
