...
use crate::intrinsics::{
    AsyncTaskIntrinsic, ComponentIntrinsic, ConversionIntrinsic, Intrinsic, RenderIntrinsicsArgs,
    WaitableIntrinsic,
};
...
impl super::BuiltinIntrinsicRenderer {
    /// Render a [`AsyncTaskIntrinsic`]
    pub(crate) fn render_async_task(
        &self,
        intrinsic: &AsyncTaskIntrinsic,
        output: &mut Source,
        ...
        match intrinsic {
            ...
            AsyncTaskIntrinsic::GlobalAsyncCurrentTaskMap => {
                let var_name = AsyncTaskIntrinsic::GlobalAsyncCurrentTaskMap.name();
            ...
            AsyncTaskIntrinsic::AsyncBlockedConstant => {
                let name = AsyncTaskIntrinsic::AsyncBlockedConstant.name();
                output.push_str(&format!("const {name} = 0xFFFF_FFFF;"));
            }

            AsyncTaskIntrinsic::ContextSet => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let context_set_fn = AsyncTaskIntrinsic::ContextSet.name();
            ...
            AsyncTaskIntrinsic::ContextGet => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let context_get_fn = AsyncTaskIntrinsic::ContextGet.name();
                let current_task_get_fn = AsyncTaskIntrinsic::GetCurrentTask.name();
                let get_global_current_task_meta_fn = Intrinsic::GetGlobalCurrentTaskMetaFn.name();
            ...
            AsyncTaskIntrinsic::TaskReturn => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let task_return_fn = AsyncTaskIntrinsic::TaskReturn.name();
                let get_global_current_task_meta_fn = Intrinsic::GetGlobalCurrentTaskMetaFn.name();
                let current_task_get_fn = AsyncTaskIntrinsic::GetCurrentTask.name();
            ...
            AsyncTaskIntrinsic::SubtaskDrop => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let subtask_drop_fn = AsyncTaskIntrinsic::SubtaskDrop.name();
            ...
            AsyncTaskIntrinsic::Yield => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let yield_fn = intrinsic.name();
                let current_task_get_fn = AsyncTaskIntrinsic::GetCurrentTask.name();
                let get_global_current_task_meta_fn = Intrinsic::GetGlobalCurrentTaskMetaFn.name();
            ...
            AsyncTaskIntrinsic::SubtaskCancel => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let task_cancel_fn = AsyncTaskIntrinsic::SubtaskCancel.name();
                let current_task_get_fn = AsyncTaskIntrinsic::GetCurrentTask.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
                let get_global_current_task_meta_fn = Intrinsic::GetGlobalCurrentTaskMetaFn.name();
            ...
            AsyncTaskIntrinsic::TaskCancel => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let task_cancel_fn = AsyncTaskIntrinsic::TaskCancel.name();
                let current_task_get_fn = AsyncTaskIntrinsic::GetCurrentTask.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
                let get_global_current_task_meta_fn = Intrinsic::GetGlobalCurrentTaskMetaFn.name();
            ...
            AsyncTaskIntrinsic::CreateNewCurrentTask => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let task_class = AsyncTaskIntrinsic::AsyncTaskClass.name();
                let global_task_map = AsyncTaskIntrinsic::GlobalAsyncCurrentTaskMap.name();
                let task_id_globals = AsyncTaskIntrinsic::GlobalAsyncCurrentTaskIds.name();
            ...
            AsyncTaskIntrinsic::GetCurrentTask => {
                let global_task_map = AsyncTaskIntrinsic::GlobalAsyncCurrentTaskMap.name();
            ...
            AsyncTaskIntrinsic::ClearCurrentTask => {
                let debug_log_fn = Intrinsic::DebugLog.name();
            ...
            AsyncTaskIntrinsic::AsyncTaskClass => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
                let event_code_enum = Intrinsic::AsyncEventCodeEnum.name();
                let task_class = AsyncTaskIntrinsic::AsyncTaskClass.name();
                let subtask_class = AsyncTaskIntrinsic::AsyncSubtaskClass.name();
            ...
            AsyncTaskIntrinsic::AsyncSubtaskClass => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let subtask_class = AsyncTaskIntrinsic::AsyncSubtaskClass.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
            ...
            AsyncTaskIntrinsic::UnpackCallbackResult => {
                let unpack_callback_result_fn = AsyncTaskIntrinsic::UnpackCallbackResult.name();
            ...
            AsyncTaskIntrinsic::DriverLoop => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let driver_loop_fn = AsyncTaskIntrinsic::DriverLoop.name();
            ...
            //
            AsyncTaskIntrinsic::LowerImport => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let lower_import_fn = AsyncTaskIntrinsic::LowerImport.name();
                let current_task_get_fn = AsyncTaskIntrinsic::GetCurrentTask.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
            ...
            AsyncTaskIntrinsic::LowerImportBackwardsCompat => {
                ...
                let lower_import_backwards_compat_fn =
                    AsyncTaskIntrinsic::LowerImportBackwardsCompat.name();
                let current_task_get_fn = AsyncTaskIntrinsic::GetCurrentTask.name();
                let create_new_current_task_fn = AsyncTaskIntrinsic::CreateNewCurrentTask.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
                let async_event_code_enum = Intrinsic::AsyncEventCodeEnum.name();
                let get_global_current_task_meta_fn = Intrinsic::GetGlobalCurrentTaskMetaFn.name();
            ...
            AsyncTaskIntrinsic::EnterSymmetricSyncGuestCall => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let enter_symmetric_sync_guest_call_fn = intrinsic.name();
                ...
                let symmetric_sync_guest_call_stack =
                    AsyncTaskIntrinsic::SymmetricSyncGuestCallStack.name();
            ...
            AsyncTaskIntrinsic::ExitSymmetricSyncGuestCall => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let exit_symmetric_sync_guest_call_fn = intrinsic.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
                let get_current_task_fn =
                    Intrinsic::AsyncTask(AsyncTaskIntrinsic::GetCurrentTask).name();
            ...
            AsyncTaskIntrinsic::SymmetricSyncGuestCallStack => {
                let var_name = intrinsic.name();
