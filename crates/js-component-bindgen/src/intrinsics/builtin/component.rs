...
use crate::intrinsics::{
    AsyncFutureIntrinsic, AsyncStreamIntrinsic, ComponentIntrinsic, Intrinsic,
    RenderIntrinsicsArgs, WaitableIntrinsic,
};
use crate::source::Source;
use crate::uwriteln;

impl super::BuiltinIntrinsicRenderer {
    /// Render a [`ComponentIntrinsic`]
    pub(crate) fn render_component(
        &self,
        intrinsic: &ComponentIntrinsic,
        output: &mut Source,
        _render_args: &RenderIntrinsicsArgs,
    ) {
        match intrinsic {
            ComponentIntrinsic::GlobalAsyncStateMap => {
                let var_name = ComponentIntrinsic::GlobalAsyncStateMap.name();
                uwriteln!(output, r#"const {var_name} = new Map();"#);
            }

            ComponentIntrinsic::BackpressureInc => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let backpressure_inc_fn = ComponentIntrinsic::BackpressureInc.name();
            ...
            ComponentIntrinsic::BackpressureDec => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let backpressure_dec_fn = ComponentIntrinsic::BackpressureDec.name();
                let get_or_create_async_state_fn = ComponentIntrinsic::GetOrCreateAsyncState.name();
                output.push_str(&format!(
                    "
            ...
            ComponentIntrinsic::ComponentAsyncStateClass => {
                let component_async_state_class = intrinsic.name();
                let debug_log_fn = Intrinsic::DebugLog.name();
            ...
            ComponentIntrinsic::GetOrCreateAsyncState => {
                let get_state_fn = ComponentIntrinsic::GetOrCreateAsyncState.name();
                let async_state_map = ComponentIntrinsic::GlobalAsyncStateMap.name();
                let component_async_state_class =
                    ComponentIntrinsic::ComponentAsyncStateClass.name();
                output.push_str(&format!(
                    r#"
            ...
            ComponentIntrinsic::ComponentStateSetAllError => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let async_state_map = ComponentIntrinsic::GlobalAsyncStateMap.name();
                let component_state_set_all_error_fn =
                    ComponentIntrinsic::ComponentStateSetAllError.name();
                output.push_str(&format!(
                    r#"
