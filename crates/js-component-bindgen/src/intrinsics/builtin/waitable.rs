use crate::intrinsics::{
    AsyncTaskIntrinsic, ComponentIntrinsic, HostIntrinsic, Intrinsic, RenderIntrinsicsArgs,
    WaitableIntrinsic,
};
use crate::source::Source;

impl super::BuiltinIntrinsicRenderer {
    /// Render a [`WaitableIntrinsic`]
    pub(crate) fn render_waitable(
        &self,
        intrinsic: &WaitableIntrinsic,
        output: &mut Source,
        _render_args: &RenderIntrinsicsArgs,
    ) {
        match intrinsic {
            WaitableIntrinsic::WaitableSetClass => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let waitable_set_class = WaitableIntrinsic::WaitableSetClass.name();
            ...
            WaitableIntrinsic::WaitableClass => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let waitable_class = WaitableIntrinsic::WaitableClass.name();
            ...
            WaitableIntrinsic::WaitableSetNew => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
                let waitable_set_class = WaitableIntrinsic::WaitableSetClass.name();
                let waitable_set_new_fn = WaitableIntrinsic::WaitableSetNew.name();
            ...
            WaitableIntrinsic::WaitableSetWait => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let waitable_set_wait_fn = WaitableIntrinsic::WaitableSetWait.name();
            ...
            WaitableIntrinsic::WaitableSetPoll => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let waitable_set_poll_fn = WaitableIntrinsic::WaitableSetPoll.name();
                let current_task_get_fn =
                    Intrinsic::AsyncTask(AsyncTaskIntrinsic::GetCurrentTask).name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
            ...
            WaitableIntrinsic::WaitableSetDrop => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let waitable_set_drop_fn = WaitableIntrinsic::WaitableSetDrop.name();
                let current_task_get_fn =
                    Intrinsic::AsyncTask(AsyncTaskIntrinsic::GetCurrentTask).name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
                let remove_waitable_set_fn = WaitableIntrinsic::RemoveWaitableSet.name();
                output.push_str(&format!("
            ...
            WaitableIntrinsic::RemoveWaitableSet => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let remove_waitable_set_fn = WaitableIntrinsic::RemoveWaitableSet.name();
                output.push_str(&format!(r#"
            ...
            WaitableIntrinsic::WaitableJoin => {
                let debug_log_fn = Intrinsic::DebugLog.name();
                let waitable_join_fn = WaitableIntrinsic::WaitableJoin.name();
                let get_or_create_async_state_fn =
                    Intrinsic::Component(ComponentIntrinsic::GetOrCreateAsyncState).name();
                output.push_str(&format!(r#"
