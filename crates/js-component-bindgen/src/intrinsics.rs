...
pub enum Intrinsic {
    ...
    /// The definition of the waitable set class
    WaitableSetClass,
    ...
    ///
    /// ```ts
    /// function waitableSetWait(isAsync, memory, waitableSetIdx, resultPtr);
    /// ```
    ///
    ...
    ///
    /// ```ts
    /// function waitableSetPoll(isAsync, memory, waitableSetIdx, resultPtr);
    /// ```
    ///
    /// The results of the poll should be set in the provided pointer
    WaitableSetPoll,
    ...
    ///
    /// ```ts
    /// function waitableSetPoll(waitableSetIdx);
    /// ```
    WaitableSetDrop,
    ...
    ///
    /// ```ts
    /// function waitableSetPoll(waitableIdx, waitableSetIdx);
    /// ```
    ///
    ...
    for i in intrinsics.iter() {
        match i {
                    ...
                    class {task_class} {{
                        ...
                        async waitForEvent(opts) {{
                            ...
                            while (event == null) {{
                                ...
                                if (waited) {{
                                    if (this.#state !== TaskState.INITIAL) {{
            ...
            Intrinsic::WaitableSetWait => {
                ...
                output.push_str(&format!("
                    async function {waitable_set_wait_fn}(componentInstanceID, memory, isAsync, waitableSetRep, resultPtr) {{
                        {debug_log_fn}('[{waitable_set_wait_fn}()] args', {{ ciid, value }});
                        const task = {current_task_get_fn}();
            ...
            Intrinsic::WaitableSetPoll => {
                ...
                let write_async_event_to_memory_fn = Intrinsic::WriteAsyncEventToMemory.name();
                output.push_str(&format!("
                    function {waitable_set_poll_fn}(componentInstanceID, memory, isAsync, waitableSetRep, resultPtr) {{
                        {debug_log_fn}('[{waitable_set_poll_fn}()] args', {{ ciid, value }});
                        const task = {current_task_get_fn}();
                        if (!task) {{ throw Error('invalid/missing async task'); }}
                        if (task.componentIdx !== componentInstanceID ) {{
                            throw Error(['task component idx [' + task.componentIdx + '] != component instance ID [' + componentInstanceID + ']');
            ...
            // TODO: implement WaitableSet.drop
            Intrinsic::WaitableSetDrop => {
                ...
                output.push_str(&format!("
                    function {waitable_set_drop_fn}(componentInstanceID, waitableSetRep) {{
                        ...
                        const task = {current_task_get_fn}();
                        if (!task) {{ throw drop Error('invalid/missing async task'); }}
                        if (task.componentIdx !== componentInstanceID ) {{
                            throw Error(['task component idx [' + task.componentIdx + '] != component instance ID [' + componentInstanceID + ']');
                        }}
                        throw drop Error('not implemented!');
                    }}
                "));
            },
